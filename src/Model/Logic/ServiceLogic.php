...
class ServiceLogic
{
    ...
    public function create(array $params): void
    {
        ...
        foreach ($tableSchemas as $tableSchema) {
            ...
            $this->generateDao($tableSchema, $pool, $basePath.'/'.$path, $isConfirm, $fieldPrefix, $tplDir, '', $path);
        }
    }
    ...
    private function generateDao(
        ...
        $extName = (empty($extName)) ? '' : '-'.$extName;
        $config       = [
            'tplFilename' => 'service'.$extName,
            'tplDir'      => $tplDir,
        ...
        $file .= sprintf('/%s%sService.php', $mappingClass, $extName);
...
//            $genGetters[] = $this->generateGetters($columnSchema, $tplDir);
        ...
        $use = sprintf($request, 'Api', $mPath, 'Api', $mappingClass, 'Create').PHP_EOL;
        $use .= sprintf($request, 'Api', $mPath, 'Api', $mappingClass, 'Update').PHP_EOL;
        $use .= sprintf($request, 'Api', $mPath, 'Api', $mappingClass, 'PaginateQuery').PHP_EOL;
        $use .= sprintf($request, 'Seller', $mPath, 'Seller', $mappingClass, 'Create').PHP_EOL;
        $use .= sprintf($request, 'Seller', $mPath, 'Seller', $mappingClass, 'Update').PHP_EOL;
        $use .= sprintf($request, 'Seller', $mPath, 'Seller', $mappingClass, 'PaginateQuery').PHP_EOL;
        $use .= sprintf('use App\Logic\Dto\Response\%s\%s\%s%s%sResponse;', "Api", $mPath, "Api", $mappingClass, "Info").PHP_EOL;
        $use .= sprintf('use App\Model\Entity\%s\%s;', $mPath, $mappingClass).PHP_EOL;
        $use .= sprintf('use App\Model\Dao\%s\%sDao;', $mPath, $mappingClass).PHP_EOL;

        $data = [
            'properties'   => $propertyStr,
            'methods'      => $methodStr,
            'tableName'    => $tableSchema['name'],
            'entityName'   => $mappingClass.$extName.'Service',
